...
fn roundtrip(b: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let obj: Ipld = serde_ipld_dagcbor::from_reader(cursor)
        .map_err(|e| format!("dag-cbor decode error: {}", e))?;

    // Encode back using dag-cbor
    let mut output = Vec::new();
    serde_ipld_dagcbor::to_writer(&mut output, &obj)
...
fn invalid_decode(b: &[u8]) -> (bool, String) {
    let cursor = Cursor::new(b);
    let result: Result<Ipld, _> = serde_ipld_dagcbor::from_reader(cursor);
...
fn invalid_encode(b: &[u8]) -> (bool, String) {
    ...
    let mut output = Vec::new();
    match serde_ipld_dagcbor::to_writer(&mut output, &obj) {
        Ok(_) => (false, String::new()),
        Err(e) => (true, e.to_string()),
    }
}
